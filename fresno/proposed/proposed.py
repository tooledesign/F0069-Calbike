...
i.import_osm_network(osm_file="/home/sgardner/fresno_reducednov05.osm",keep_holding_tables=True)
...
bna.config.bna.connectivity.table = "automated.fresno_proposed_connected_blocks_3mi"
...
q_insert = """
    insert into automated.fresno_proposed_aggregate_scores
    select * from {};
"""
bna._run_sql(q_id.format("automated.fresno_proposed_aggregate_scores"))
for project in projects:
    bna_table = "automated.fresno_proposed_bna_scores_{}".format(project.replace(" ","_").lower())
    agg_table = "scratch.fresno_agg_{}".format(project.replace(" ","_").lower())
...
bna._run_sql("alter table automated.fresno_proposed_aggregate_scores add column id serial primary key;")
